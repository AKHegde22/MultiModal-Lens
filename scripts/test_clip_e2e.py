"""End-to-end integration script to test CLIP (openai/clip-vit-base-patch32) with MultiModal-Lens."""
...
def main():
    print("=" * 60)
    print("Starting End-to-End Verification with CLIP (openai/clip-vit-base-patch32)")
    print("=" * 60)
    ...
    # 1. Instantiate HookedVLM
    print("\n1. Loading pre-cached CLIP model via HookedVLM...")
    ...
    print(f"  - Tokens ({len(analysis_res.tokens)}): {analysis_res.tokens}")
    ...
    print(f"  - Alignment matrix min/max/mean: {analysis_res.alignment_matrix.min():.4f} / {analysis_res.alignment_matrix.max():.4f} / {analysis_res.alignment_matrix.mean():.4f}")
    ...
    vll_res = run_vision_logit_lens(adapter, image, top_k=3)
    ...
    dla_html_path = "outputs/clip_dla.html"
    dla_res.save_html(dla_html_path)

    dla_plotly_path = "outputs/clip_dla_plotly.html"
    dla_fig = dla_res.to_plotly()
    dla_fig.write_html(dla_plotly_path)

    path_html_path = "outputs/clip_path_patching.html"
    path_res.save_html(path_html_path)

    path_plotly_path = "outputs/clip_path_patching_plotly.html"
    ...
    print("\n" + "=" * 60)
    print("ALL END-TO-END TESTS PASSED FOR CLIP (openai/clip-vit-base-patch32)!")
    print("=" * 60)
