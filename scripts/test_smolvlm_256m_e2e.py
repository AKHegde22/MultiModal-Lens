"""End-to-end integration script to test SmolVLM-256M-Instruct with MultiModal-Lens."""
...
def main():
    print("=" * 60)
    print("Starting End-to-End Verification with SmolVLM-256M-Instruct (0.25B)")
    print("=" * 60)

    model_name = "HuggingFaceTB/SmolVLM-256M-Instruct"
    device = "cpu"  # Fast and reliable for 256M
    print(f"Device: {device}")

    # 1. Instantiate Adapter
    print("\n1. Loading SmolVLM-256M-Instruct adapter...")
    ...
    # 7. Test HTML and Plotly exports
    print("\n7. Testing Dashboard and Plotly Exports...")
    dla_html = dla_res.to_html()
    dla_fig = dla_res.to_plotly()
    path_html = path_res.to_html()
    path_fig = path_res.to_plotly()
    print(f"  - DLA HTML length: {len(dla_html)} chars")
    print(f"  - DLA Plotly Figure type: {type(dla_fig)}")
    print(f"  - Path Patching HTML length: {len(path_html)} chars")
    print(f"  - Path Patching Plotly Figure type: {type(path_fig)}")

    print("\n" + "=" * 60)
    print("ALL END-TO-END TESTS PASSED FOR SmolVLM-256M-Instruct!")
    print("=" * 60)
